from processpi.pipelines.engine import PipelineEngine
from processpi.pipelines.pipes import Pipe
from processpi.pipelines.network import PipelineNetwork
from processpi.units import *
from processpi.components import Water

# -----------------------------
# Cooling Tower Network Parameters
# -----------------------------
fluid = Water(temperature=Temperature(30, "C"), pressure=Pressure(101325, "Pa"))

flow_rate_per_condenser_m3h = VolumetricFlowRate(50, "m3/h")
flow_rate_per_condenser_m3s = flow_rate_per_condenser_m3h.to("m3/s")
#print(flow_rate_per_condenser_m3s)
num_condensers = 8

total_flow_rate_m3s = flow_rate_per_condenser_m3s.value * num_condensers
#print(total_flow_rate_m3h)
total_flow_rate_m3s = VolumetricFlowRate(total_flow_rate_m3s)
branch_flow_m3s = flow_rate_per_condenser_m3s


# -----------------------------
# Build main network - Using alternative fit method
# -----------------------------
# Main header pipe (auto-size)
header_pipe = Pipe(
    name="Main Header",
    length=Length(100, "m"),
    # diameter=None,          # auto-size - Removing diameter for fit to handle sizing
    flow_rate=total_flow_rate_m3s,
    roughness=0.000045,
    schedule="STD"
)

# Parallel branches - Using alternative fit method
branch_pipes = []
for i in range(num_condensers):
    branch_pipe = Pipe(
        name=f"Branch Pipe {i+1}",
        length=Length(30, "m"),
        diameter=Diameter(2,"in"),       # Keeping specified diameter for branches
        flow_rate=branch_flow_m3s,
        roughness=0.000045,
        schedule="STD"
    )
    # Set a non-zero initial flow to avoid Re=0 in solver - This might not be needed with the new fit approach
    # branch_pipe.flow_rate = branch_flow_m3s
    branch_pipes.append(branch_pipe)


# Initialize engine and fit with components directly
engine = PipelineEngine()
engine.fit(
    # network=network, # Removing network argument
    fluid=fluid,
    flow_rate=total_flow_rate_m3s,
    pipes=[header_pipe] + branch_pipes # Passing pipes directly
)

# Optional: ensure all pipes have initial flows - This might not be needed with the new fit approach
# for branch in engine._normalize_branches(network):
#     for pipe in branch:
#         if pipe.flow_rate is None:
#             pipe.flow_rate = VolumetricFlowRate(0.001, "m3/s")  # small non-zero guess

# Run solver
results = engine.run()

# Print summaries
print("\n--- Cooling Tower Network Results ---")
#engine.summary()
#results.detailed_summary()
results.summary()